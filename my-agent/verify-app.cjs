...
async function runTests() {
  ...
  try {
    ...
    const headline = await page.$eval('h1', function(el) { return el.textContent; }).catch(function() { return null; });
    ...
    const heroSection = await page.$('section, [class*="hero"], main > div:first-child').catch(function() { return null; });
    ...
    await page.waitForSelector('form', { timeout: 10000 }).catch(function() {});
    ...
    for (var i = 0; i < buttons.length; i++) {
      var btn = buttons[i];
      var text = await btn.evaluate(function(el) { return el.textContent; });
    ...
    await page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 10000 }).catch(function() {});
