...
(async () => {
  ...
  await page.screenshot({ path: SCREENSHOTS_DIR + '/feat-001-step1-navigate.png', fullPage: true });
  ...
  await page.type('input[name="full_name"]', 'John Test Doe');
  const nameValue = await page.$eval('input[name="full_name"]', el => el.value);
  if (nameValue !== 'John Test Doe') throw new Error('Name field did not accept text');
  ...
  await page.type('input[name="email"]', 'john.doe@test.com');
  ...
  await page.type('input[name="phone"]', '301-555-1234');
  const phoneValue = await page.$eval('input[name="phone"]', el => el.value);
  console.log('✓ Phone field accepts phone: ' + phoneValue);
  
  await page.screenshot({ path: SCREENSHOTS_DIR + '/feat-001-step4-all-fields-filled.png', fullPage: true });
  ...
  await page.screenshot({ path: SCREENSHOTS_DIR + '/feat-001-step6-validation-check.png', fullPage: true });
